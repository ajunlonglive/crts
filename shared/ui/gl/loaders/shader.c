...
#include "shared/ui/gl/loaders/shader.h"
...
compile_shader(const char *path, GLenum type, uint32_t *id)
{
	...
	*id = glCreateShader(type);
	...
	if (!ret) {
		...
		LOG_W(log_gui, "failed to compile '%s'\n%s", path, buf);
		return false;
	}
...
link_shaders(struct shader_src *shaders, uint32_t *program)
{
	...
	*program = glCreateProgram();
	...
	if (!ret) {
		...
		LOG_W(log_gui, "failed to link program\n%s", errinfo);
		LOG_W(log_gui, "program:");
		for (i = 0; shaders[i].path && *shaders[i].path; ++i) {
			LOG_W(log_gui, "--> %s", shaders[i].path);
		}
		return false;
	}
