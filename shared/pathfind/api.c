...
{
	L(log_pathfind, "initializing ag, %ld", sizeof(struct ag_key));
...
hpa_finish(struct chunks *cnks, uint32_t path_id)
{
	...
	if (path->flags & ppf_initialized) {
		/* L(log_pathfind, "pruning path %d", path_id); */
	...
	uint32_t *handle)
{
	...
	if ((free_len = darr_len(&cnks->ag.free_paths))) {
		...
		/* L(log_pathfind, "creating new path slot @ %d", id); */
		path = darr_get_mem(&cnks->ag.paths);
	}

	/* L(log_pathfind, "got path handle: %d", id); */
...
hpa_continue(struct chunks *cnks, uint32_t id, struct point *p)
{
	...
	if (path->flags & ppf_local_done) {
		...
		/* L(log_pathfind, "(%d, %d) | (%d, %d) -> (%d, %d)", cur_cmp->x, cur_cmp->y, */
		/* 	cur_node >> 4, */
		/* 	cur_node & 15, */
		/* 	nxt_node >> 4, */
		/* 	nxt_node & 15 */
		...
		if (!astar_local(agc, cur_node, nxt_node,
			...
			LOG_W(log_pathfind, "failed to pathfind locally, the abstract path is invalid");
			assert(false);
		}
	...
	if (path->local.i) {
		...
		if (path->flags & ppf_abstract_done) {
			/* L(log_pathfind, "done pathfining, calling hpa_finish"); */
			hpa_finish(cnks, id);
...
hpa_clean(struct chunks *cnks)
{
	...
	for (i = 0; i < hdarr_len(&cnks->ag.dirty); ++i) {
		...
		if (dirt->type & dt_reset) {
			...
			/* L(log_pathfind, "resetting component, (%d, %d)", agc->pos.x, agc->pos.y); */
	...
	for (i = 0; i < hdarr_len(&cnks->ag.dirty); ++i) {
		...
		if (dirt->type & dt_relink) {
			...
			/* L(log_pathfind, "relinking component, (%d, %d)", agc->pos.x, agc->pos.y); */
