...
unpack_cm_action(struct cm_action *au, const char *buf)
{
	...
	b += unpack_uint16_t(&au->workers, &buf[b]);
	b += unpack_uint8_t(&au->flags, &buf[b]);
	b += unpack_rectangle(&au->range, &buf[b]);
	b += unpack_rectangle(&au->source, &buf[b]);
...
pack_cm_action(const struct cm_action *au, char *buf)
{
	...
	b += pack_uint16_t(&au->workers, &buf[b]);
	b += pack_uint8_t(&au->flags, &buf[b]);
	b += pack_rectangle(&au->range, &buf[b]);
	b += pack_rectangle(&au->source, &buf[b]);
