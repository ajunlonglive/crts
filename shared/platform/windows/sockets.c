...
wsock_resolve(struct sock_addr *addr, const char *host)
{
	...
	L(log_net, "resolved %s to %s", resp->ai_canonname, inet_ntoa(saddr.ia->sin_addr));
...
wsock_bind(struct sock_addr *addr, sock_t *sock)
{
	...
	if (isock == INVALID_SOCKET) {
		L(log_net, "failed to create socket: %s", win_strerror());
		return false;
	}
	...
	*sock = isock;
	...
	if ((ret = bind(*sock, &saddr.sa, socklen)) == SOCKET_ERROR) {
		L(log_net, "failed to bind socket: %s", win_strerror());
		return false;
	}

	L(log_net, "successfully bound socket @ %s", sock_addr_to_s(addr));

	u_long non_block = 1;
	if (ioctlsocket(*sock, FIONBIO, &non_block) == SOCKET_ERROR) {
		L(log_net, "failed to set nonblocking mode: %s", win_strerror());
		return false;
	}
