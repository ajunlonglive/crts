...
void gl_write_char(float x, float y, const vec4 clr, char c);
size_t gl_write_string(float x, float y, float scale, const vec4 clr, const char *str);
size_t gl_printf(float x, float y, enum text_anchor anch, const char *fmt, ...);
size_t gl_write_string_centered(float x, float y, float scale, const vec4 clr,
	const char *str);
...
typedef bool ((*interactive_text_cb)(void *ctx, bool hover, float x, float y, const char *str));

bool gl_mprintf(float sx, float sy, enum text_anchor anch, const struct pointf *c,
